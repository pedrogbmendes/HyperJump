...
class GaussianProcess(BaseModel):
    ...
    def train(self, X, y, do_optimize=True):
        ...
        #print("GP Hyperparameters: " + str(self.hypers))

        try:
            self.gp.compute(self.X, yerr=np.sqrt(self.noise))
        except np.linalg.LinAlgError:
            #print(self.X)
            #print(np.sqrt(self.noise))
            self.noise *= 10
            self.gp.compute(self.X, yerr=np.sqrt(self.noise))
    ...
    def get_incumbent_with_budget(self, budget):
        ...
        incumbent_value: ndarray (N,)
            the observed value of the incumbent
        """
        budget, _, _ = normalization.zero_one_normalization(budget, self.lower, self.upper)
        inc, inc_value = super(GaussianProcess, self).get_incumbent_with_budget(budget)
        if inc_value == -1: return -1, -1
        if self.normalize_input:
            inc = normalization.zero_one_unnormalization(inc, self.lower, self.upper)
