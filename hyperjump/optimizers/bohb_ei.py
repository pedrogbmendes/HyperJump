...
from hyperjump.optimizers.config_generators.bohb_ei import BOHB_EI as CG_BOHB

class BOHB_EI(Master):
	...
	def get_next_iteration(self, iteration, iteration_kwargs={}, hyperjump=False):
		...
		#print("Current incumbent = {}, loss = {:.4f}".format(self.config_generator.incumbent, self.config_generator.incumbent_value))
		#print(ns)
		res = (SuccessiveHalving(HPB_iter=iteration, num_configs=ns, budgets=self.budgets[(-s-1):], config_sampler=self.config_generator, **iteration_kwargs))
		counter = 0
		return res
