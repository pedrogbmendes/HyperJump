...
from hyperjump.core.dispatcher import Dispatcher
from hyperjump.core.result import Result
from hyperjump.core.base_iteration import WarmStartIteration
...
class Master(object):
	def __init__(self,
		...
		config_generator: hyperjump.config_generators object
		...
		previous_result: hyperjump.core.result.Result object
		...
		if logger is None:
			self.logger = logging.getLogger('hyperjump')
		else:
			self.logger = logger
	...
	def run(self, n_iterations=1, min_n_workers=1, iteration_kwargs = {},):
		...
		#print("[MASTER] Starting the optimization..")
		...
		while True:
			...
			# print("[MASTER] - Finding a new run to schedule..")
			# print("           Current iterations length : ", len(self.iterations))
			
			next_run = None
			#self.Hyperjump()
                
			#print("[ACTIVE Iterations] : ", self.active_iterations())
			# find a new run to schedule

			for i in self.active_iterations():
				#print("[MASTER] - Fetching new run..")
				self.iterations[i].update_initTime()
				next_run = self.iterations[i].get_next_run()
				if not next_run is None: break

			if not next_run is None:
				#print("[MASTER] - Submiting a new run ")# next_run
				self.logger.debug('HBMASTER: schedule new run for iteration %i'%i)
				self._submit_job(*next_run)
				self.no_tested_configs+=1
				continue
			else:
				#print("[MASTER] - Finished an iteration!")
				#stop condition is number of evaluations
				

				#############################
				# stop condition
				############################
				#if self.no_tested_configs <= 1024:
				#if self.no_tested_configs <= 256:
				if self.no_tested_configs <= 500:
					self.iterations.append(self.get_next_iteration(len(self.iterations), iteration_kwargs, hyperjump=self.hyperjump))
					continue
	
			
				#stop condition is number of iterations
				#if n_iterations > 0:	#we might be able to start the next iteration
				#	self.iterations.append(self.get_next_iteration(len(self.iterations), iteration_kwargs, hyperjump=self.hyperjump))
				#	n_iterations -= 1
				#	continue

			# at this point there is no ime diate run that can be scheduled,
