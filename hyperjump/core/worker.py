...
class Worker(object):
	...
	def __init__(self, run_id, nameserver=None, nameserver_port=None, logger=None, host=None, id=None, timeout=None):
		...
		run_id: anything with a __str__ method
			unique id to identify individual hyperjump run
		nameserver: str
		...
		self.worker_id =  "hyperjump.run_%s.worker.%s.%i"%(self.run_id, socket.gethostname(), os.getpid())
	...
	def load_nameserver_credentials(self, working_directory, num_tries=60, interval=1):
		...
		for i in range(num_tries):
			try:
				...
				print("ERROR: load_nameserver_credentials 1")
				raise
	...
	def _run(self):
		...
		try:
			with Pyro4.locateNS(host=self.nameserver, port=self.nameserver_port) as ns:
				...
				dispatchers = ns.list(prefix="hyperjump.run_%s.dispatcher"%self.run_id)
		except Pyro4.errors.NamingError:
			if self.thread is None:
				print("ERROR: worker _run 1")
	...
	def compute(self, config_id, config, budget, working_directory):
		...
		raise NotImplementedError("Subclass hyperjump.distributed.worker and overwrite the compute method in your worker script")
	...
	def start_computation(self, callback, id, *args, **kwargs):
		...
		try:
			...
			print("ERROR: start_computation _run 1")
			result = {'result': None,
						'exception' : traceback.format_exc()}
		finally:
			self.logger.debug('WORKER: done with job %s, trying to register it.'%str(id))
			#print('WORKER: done with job %s, trying to register it.'%str(id))
			#print(result)
			with self.thread_cond:
				self.busy =  False
