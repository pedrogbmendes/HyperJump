...
class Worker(object):
	...
	def is_alive(self):
		try:
			...
			print("ERROR: is_alive _run 1")
			return False
		except:
			print("ERROR: is_alive _run 2")
			raise
		return(True)
...
class Dispatcher(object):
	...
	def __init__(self, new_result_callback, run_id='0',
		...
		new_result_callback: function
		    function that will be called with a `Job instance <hyperjump.core.dispatcher.Job>`_ as argument.
		...
		self.pyro_id="hyperjump.run_%s.dispatcher"%self.run_id
	...
	def discover_workers(self):
		...
		while True:
			...
			with Pyro4.locateNS(host=self.nameserver, port=self.nameserver_port) as ns:
				worker_names = ns.list(prefix="hyperjump.run_%s.worker."%self.run_id)
			...
			#	print('No workers available! Keep pinging')
	...
	def register_result(self, id=None, result=None):
		...
		with self.runner_cond:
			...
			try:
				...
				print("ERROR: register_result _run 1")
				pass
			except:
				print("ERROR: register_result _run 2")
				raise
