...
class csv_results(object):
	def __init__(self, directory, seed, max_budget, factor=1.0, overwrite=False, logger=None):
		self.logger = logger

		os.makedirs(directory, exist_ok=True)

		self.file_logs = os.path.join(directory, 'logs.csv')
		self.file_logs_aux = os.path.join(directory, 'logs_aux.csv')
		...
		try:
			...
			self.logger.error("ERROR: logs already exist. please backup or delete file")
			raise
		
		try:
			with open(self.file_logs_aux, 'x') as fh: pass
		except FileExistsError:
			if overwrite:
				with open(self.file_logs_aux, 'w') as fh: pass
			else:
				raise FileExistsError('The file %s already exists.'%self.file_logs_aux)
		except:
			self.logger.error("ERROR: logs_aux already exist. please backup or delete file")
			raise


		with open(self.file_logs, 'a') as fh:
			fh.write("runID;it;incumbent;incTime;incAcc;budget;configTested;Time;Acc;StartTime;EndTime;RealTime;Overhead\n")

		with open(self.file_logs_aux, 'a') as fh:
			fh.write("runID;it;incumbent;incTime;incAcc;budget;configTested;Time;Acc;StartTime;EndTime;RealTime;Overhead\n")
	...
	def new_config(self, config_id, config, config_info, overhead=0):
		#new config is deployed to run
		if not config_id in self.config_ids:
			self.config_ids.add(config_id)
			self.configs2Test.append([config_id, config, config_info, overhead])
	...
	def __call__(self, job):
		...
		if not job.id in self.config_ids:
			...
			if self.logger is not None:
				self.logger.error("JOB id was not submitted first.")
			self.config_ids.add(job.id)


		config_info = job.result["info"]
		conf = config_info["config"]
		confTime = config_info["training_time"]
		confAcc = config_info["accuracy"]
		budget = job.kwargs['budget']
	
		starttime = config_info["start_time"]
		t_now = config_info["end_time"]
		real_time = config_info["real_time"]
		overhead = 0 #config_info["overhead"]

		with open(self.file_logs_aux, 'a') as fh:
			strWrite = str(self.seed) + ";-1;-1;-1;" + str(budget) + ";" + str(conf) + ";" + str(confTime) + ";" + str(confAcc) + ";" \
				+ str(starttime) + ";" + str(t_now) +  ";" + str(real_time) +  ";" + str(overhead) + "\n"
			fh.write(strWrite)
	...
	def write_results(self):
		...
		def _sort(val):
			return float(val[8]) # sort by initial  time

		def _sort1(val):
			return float(val[9]) # sort by final time

		data = []
		with open(self.file_logs_aux, 'r') as fh:
			for line in fh:
				if "runID" in line: continue
				l_aux = []
				aux_str = line.split(";")

				for field in aux_str:
					l_aux.append(field)
				data.append(l_aux)
		...
		aux_data.sort(key=_sort1)

		tinit = float(aux_data[0][8])
		prevTime = tinit
		...
		with open(self.file_logs, 'a') as fh:
			for aux_str in data:
				...
				budget = float(aux_str[4])
				config = aux_str[5]
				trainTime = float(aux_str[6])
				acc = float(aux_str[7])

				starttime = float(aux_str[8])
				fineltime = float(aux_str[9])
				real_time = float(aux_str[10][:-1]) # remove \n

				_time = (fineltime - prevTime) * self.factor
				prevTime = fineltime

				if budget == self.max_budget and acc > incAcc:
						inc = deepcopy(config)
						inc = config
						incTime = trainTime
						incAcc = acc
				...
				strWrite = str(seed) + ";" + str(it) + ";" + str(inc) + ";" + str(incTime) + ";" + str(incAcc) + ";" +  \
					str(budget) + ";" + str(config) + ";" + str(_time) + ";" + str(acc) + ";"+ str(starttime) + ";" + str(fineltime) + ";" + str(real_time) + "\n"
				fh.write(strWrite)

				it += 1

	def returnIncumbent(self):
		return self.inc, self.inc_id, self.incAcc, self.incTime 
