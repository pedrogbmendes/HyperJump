...
class FakeWorker(Worker):
    def __init__(self, budget_type='accuracy', seed=0, dataset='cnn', **kwargs):
        self.budget_type = budget_type
        self.rng = np.random.RandomState(np.int64(seed))
        self.total_cost = 0
        self.seed = seed
        self.df_table = pd.read_csv('files/' +  dataset + '.csv')
        super().__init__(**kwargs)

        self.dictConfigs = []
        self.listConfigs = self.load_dataset('files/' +  dataset + '.csv')
    ...
    def load_dataset(self, CSV):
        ...
        with open(CSV) as csv_file:
            ...
            for row in csv_reader:
                ...
                listConfig.append([nr_ps, nr_workers, learning_rate, batch_size, synchronism, vm_flavor, size])
                conf_dict = dict([
                    ('vm_flavor', flavor),
    ...
    def compute(self, config, budget, working_directory, *args, **kwargs):
        ...
        sub_df = df.loc[
            ...
            (df['training_set_size'] == int(real_budget))]
    ...
    def get_configspace(seed):
        ...
        cs.add_hyperparameters([num_cores, learning_rate, batch_size, synchronism, vm_flavor])
